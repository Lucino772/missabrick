...
def set_exists(set_number: str):
    stmt = sa.select(sa.func.count()).select_from(
        sa.select(Set).filter_by(set_num=set_number).subquery()
    )
...
def _fetch_inventories_from_sets(set_number: str, quantity: int = 1):
    ...
    inv: Inventory = db.session.execute(
        sa.select(Inventory)
        .filter_by(set_id=_set.set_num)
        .order_by(Inventory.version)
    ).scalar_one()

    inventories = [(inv, quantity)]
    for inv_set in inv.inventory_sets:
        inventories += _fetch_inventories_from_sets(
            inv_set._set.set_num, inv_set.quantity * quantity
        )
...
def get_set_data(set_number: str, quantity: int = 1):
        ...
        for inv_fig in inv_set.inventory_minifigs:
            fig_parts.append(
                (
                    ...
                    db.session.execute(
                        inv_fig.minifig.inventories.order_by("version")
                    )
                    .scalar_one()
                    .inventory_parts,
    ...
    _elements = pd.DataFrame(
        _format_elements(parts + fig_parts),
...
def gen_report(
    parts: pd.DataFrame, fig_parts: pd.DataFrame, elements: pd.DataFrame
):
...
def search_sets(search: str, current_page: int, page_size: int):
    page = db.paginate(
        sa.select(Set)
        .filter(Set.set_num.contains(search.strip()))
        .order_by(Set.year),
        page=current_page,
        per_page=page_size,
    )
