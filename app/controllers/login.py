from flask import flash, session, url_for

from app.controllers.abstract import AbstractController
from app.errors import (
    EmailVerificationError,
    InvalidEmailOrPassword,
    PasswordDoesNotMatch,
    UserAlreadyExists,
)
from app.forms.login import SignInForm, SignUpForm
from app.interfaces.controllers.login import ILoginController
from app.interfaces.views.login import ILoginView


class LoginController(AbstractController[ILoginView], ILoginController):
    def signin(self):
        form = SignInForm()
        if not form.is_submitted():
            return self.view.render("signin.html", form=form, error=None)

        error = None
        if form.validate():
            try:
                user_service = self.service_factory.get_user_service()
                user_service.check_password(
                    form.email.data, form.password.data
                )
                session["authenticated"] = True
                return self.view.redirect(url_for("explore.index"))
            except InvalidEmailOrPassword:
                error = "The email or password is incorrect"

        return self.view.render("signin.html", form=form, error=error), 422

    def signup(self):
        form = SignUpForm()
        if not form.is_submitted():
            return self.view.render("signup.html", form=form, error=None)

        error = None
        if form.validate():
            try:
                user_service = self.service_factory.get_user_service()
                user_service.create_user(
                    username=form.username.data,
                    email=form.email.data,
                    password=form.password.data,
                    confirm=form.confirm.data,
                )
                session["authenticated"] = True
                return self.view.redirect(url_for("explore.index"))
            except PasswordDoesNotMatch:
                error = "The confirm password does not match the password"
            except UserAlreadyExists:
                error = "This email or username is already used"

        return self.view.render("signup.html", form=form, error=error), 422

    def signout(self):
        if session.get("authenticated", False) is True:
            session["authenticated"] = False

        return self.view.redirect(url_for("explore.index"))

    def verify_email(self, token: str):
        error = None
        try:
            user_service = self.service_factory.get_user_service()
            user_service.verify_email(token)
            flash("Your email was verified", category="info")
            return self.view.redirect(url_for("explore.index"))
        except EmailVerificationError as err:
            if err.invalid_email:
                error = "The email you tried to verify is invalid"
            elif err.timeout:
                error = "The link expired"
            else:
                error = "Could not verify the email address"

        flash(error, category="error")
        return self.view.redirect(url_for("explore.index"))
